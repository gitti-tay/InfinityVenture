...
export function LegalAcceptanceModal({ open, onComplete, onDismiss, requireAll = true }: LegalAcceptanceModalProps) {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
        <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl">
          ...
          <div className="p-4 border-t">
            <button onClick={() => setViewDoc(null)} className="w-full py-2.5 bg-indigo-600 text-white rounded-xl font-medium hover:bg-indigo-700 transition">
              Close
            </button>
          </div>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
        <div className="p-5 border-b text-center relative">
          {onDismiss && (
            <button
              onClick={onDismiss}
              className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition"
              aria-label="Close"
            >
          ...
          <div className="p-5 space-y-3">
            <label className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-500 mt-0.5">
                  {status?.terms?.accepted ? `✅ Accepted (v${status.terms.currentVersion})` : `Version ${status?.terms?.currentVersion || '1.0'}`}
                </div>
              </div>
              <button onClick={() => viewDocument('terms')} className="text-indigo-600 text-xs font-medium hover:underline">Read</button>
            </label>
            ...
            <label className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-500 mt-0.5">
                  {status?.privacy?.accepted ? `✅ Accepted (v${status.privacy.currentVersion})` : `Version ${status?.privacy?.currentVersion || '1.0'}`}
                </div>
              </div>
              <button onClick={() => viewDocument('privacy')} className="text-indigo-600 text-xs font-medium hover:underline">Read</button>
            </label>
            ...
            <label className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-500 mt-0.5">
                  {status?.risk?.accepted ? `✅ Accepted (v${status.risk.currentVersion})` : `Version ${status?.risk?.currentVersion || '1.0'}`}
                </div>
              </div>
              <button onClick={() => viewDocument('risk_disclosure')} className="text-indigo-600 text-xs font-medium hover:underline">Read</button>
            </label>

            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-800">
              ⚠️ By checking above and clicking "Accept All", you confirm that you have read and agree to all documents. Investment involves risk of loss.
            </div>
          </div>
        )}
