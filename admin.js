...
router.post('/users/:userId/adjust-balance', superadminRequired, (req, res) => {
  ...
  const op = type === 'debit' ? '-' : '+';
  db.prepare(`UPDATE wallets SET balance = balance ${op} ?, updated_at = datetime('now') WHERE user_id = ?`)
...
router.put('/projects/:projectId', superadminRequired, (req, res) => {
  ...
  const map = {name:'name',symbol:'symbol',location:'location',category:'category',region:'region',description:'description',image:'image',series:'series',badge:'badge',riskLevel:'risk_level',targetAmount:'target_amount',minInvestment:'min_investment',maxInvestment:'max_investment',isActive:'is_active',isFeatured:'is_featured'};
  const u=[],p=[];
  for (const [k,col] of Object.entries(map)) { if (req.body[k]!==undefined) { u.push(`${col}=?`); p.push(req.body[k]); } }
  if (req.body.plans!==undefined) { u.push('plans=?'); p.push(typeof req.body.plans==='string'?req.body.plans:JSON.stringify(req.body.plans)); }
  if (!u.length) return res.status(400).json({ error: 'No fields' });
  u.push("updated_at=datetime('now')"); p.push(req.params.projectId);
  db.prepare(`UPDATE projects SET ${u.join(',')} WHERE id=?`).run(...p);
