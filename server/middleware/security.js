    ...
    return {
          origin: (origin, callback) => {
            ...
            if (ALLOWED_ORIGINS.includes(origin)) return callback(null, true);
                  // ★ SECURITY FIX: Removed wildcard *.up.railway.app — only allow explicit origins
...
export function generateCsrfToken(req, res, next) {
    ...
    res.cookie('_csrf', token, {
          httpOnly: false, sameSite: 'strict', secure: isProd,
          maxAge: 3600_000, path: '/api',
    });
    next();
}
...
const DANGEROUS_PATTERNS = [
    ...
    /<svg[\s>].*?onload/gi, /<img[^>]+onerror/gi, /expression\s*\(/gi,
    /url\s*\(\s*['"]?javascript/gi, /<base[\s>]/gi, /<form[\s>].*?action/gi,
  ];
...
function cleanupStores() {
    ...
    for (const [k, d] of bruteforceStore) {
          if (now > d.blockUntil && now - d.lastFailure > 3600_000) bruteforceStore.delete(k);
    }
}
                ...
                INSERT INTO audit_logs (id, user_id, action, resource_type, resource_id, details, ip_address, user_agent)
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                          `).run(id, userId || null, action, resourceType || null, resourceId || null,
                                            detailStr, req?.ip || null, req?.headers?.['user-agent']?.substring(0, 512) || null);
...
export function errorSanitizer(err, req, res, next) {
    ...
    const safeMsg = (status >= 500 && isProd) ? 'Internal server error'
    ...
    res.status(status).json({
          error: safeMsg, requestId: req.requestId,
