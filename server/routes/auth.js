...
import { auditLog, validateEmail, validatePassword, authLimiter, bruteforceProtection, sensitiveOpLimiter } from '../middleware/security.js';
...
router.post('/login', authLimiter, bruteforceProtection, async (req, res) => {
  try {
    ...
    if (!validPassword) {
      ...
      // Record progressive bruteforce on IP
      if (req.recordBruteforce) req.recordBruteforce();
      return res.status(401).json({ error: 'Invalid email or password' });
    }

    // Success — reset attempts, update last login + clear bruteforce
